...
export default function EstimatePage() {
  ...
  const [headerForm, setHeaderForm] = useState({ preparedBy: "", termsOfPayment: "", poNumber: "", notes: "" });
    ...
    <div className="space-y-5">
      ...
      <Card className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Field label={bi("Prepared by", "أُعد بواسطة")}>
            <Input
      ...
      <Card className="space-y-4">
        ...
        {jobEstimateLines.length > 0 && (
          <div className="overflow-x-auto rounded-lg border [border-color:var(--color-border)]">
            <table className="w-full min-w-[820px] text-sm">
              ...
              <tbody>
                {jobEstimateLines.map((line) => (
                  <tr key={line.id} className="border-t transition-colors [border-color:var(--color-border)] hover:bg-black/[0.015] dark:hover:bg-white/[0.02]">
                    ...
                    <td className="px-3 py-2 align-top text-right font-medium tabular-nums">{formatCurrency(line.totalPrice)}</td>
                    {editable && (
                      <td className="px-3 py-2 align-top text-right">
              ...
              <tfoot>
                <tr className="border-t [border-color:var(--color-border)]"><td colSpan={5} className="px-3 py-2 text-right text-[var(--color-ink-muted)]">{bi("Subtotal (excl. VAT)", "الإجمالي (غير شامل الضريبة)")}</td><td className="px-3 py-2 text-right tabular-nums">{formatCurrency((job.estimateAmount ?? 0) / 1.15)}</td></tr>
                <tr className="border-t [border-color:var(--color-border)]"><td colSpan={5} className="px-3 py-2 text-right text-[var(--color-ink-muted)]">{bi("VAT (15%)", "ضريبة القيمة المضافة (15%)")}</td><td className="px-3 py-2 text-right tabular-nums">{formatCurrency((job.estimateAmount ?? 0) - (job.estimateAmount ?? 0) / 1.15)}</td></tr>
                <tr className="border-t font-semibold [border-color:var(--color-border)]"><td colSpan={5} className="px-3 py-2 text-right">{bi("Net (incl. VAT)", "الصافي (شامل الضريبة)")}</td><td className="px-3 py-2 text-right tabular-nums">{formatCurrency(job.estimateAmount)}</td></tr>
              </tfoot>
            </table>
          </div>
        )}
        ...
        {canAddLine && (
          <div className="space-y-3 rounded-lg border [border-color:var(--color-border)] bg-[var(--color-surface-1)] p-3">
            ...
            <div className="grid gap-2 sm:grid-cols-[100px_130px_1fr_70px_130px_auto] sm:items-end">
                ...
                onClick={() => {
                  ...
                  showResult(
                    addEstimateLine(job.id, {
                      ...
                      itemId: newLineKind === "part" ? (newLineItemId || undefined) : undefined, qty: Number(newLineQty), unitPrice, notes: newLineNotes || undefined,
                    }),
                    () => { setNewLineLabel(""); setNewLineDescriptionAr(""); setNewLineCatNo(""); setNewLineItemId(""); setNewLineQty("1"); setNewLineUnitPrice(""); setNewLineNotes(""); }
                  );
                }}
              >
