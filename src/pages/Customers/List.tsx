  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 animate-rise-in">
        ...
        {canPerform(role, "create_customer") && <Button onClick={() => { setForm((current) => ({ ...current, branchId: defaultBranchId })); setOpen(true); }}>+ {bi("Add Customer", "إضافة عميل")}</Button>}
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={bi("Add Customer", "إضافة عميل")}>
        <div className="space-y-3"><Field label={bi("Full name", "الاسم الكامل")}><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label={bi("Phone", "الهاتف")}><Input value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></Field><Field label={bi("WhatsApp", "واتساب")}><Input value={form.whatsapp} onChange={(event) => setForm({ ...form, whatsapp: event.target.value })} /></Field><Field label={bi("Email", "البريد الإلكتروني")}><Input value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></Field><Field label={bi("Address", "العنوان")}><Input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} /></Field><Field label={bi("Branch", "الفرع")}><select value={form.branchId} onChange={(event) => setForm({ ...form, branchId: event.target.value })} className="w-full rounded-lg border bg-[var(--color-surface-2)] px-3 py-2 text-sm outline-none [border-color:var(--color-border)]">{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></Field><Button className="w-full justify-center" onClick={submit}>{bi("Save Customer", "حفظ العميل")}</Button></div>
      </Modal>
    </div>
  );
}
