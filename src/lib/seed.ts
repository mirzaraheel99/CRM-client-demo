...
export const CUSTOMERS: Customer[] = Array.from({ length: 60 }, (_, i) => {
  ...
  const companyName = isCorporate ? `${familyName} Trading Est.` : undefined;
  return {
    id: id("cust", i + 1),
    documentNo: `CUST-${String(i + 1).padStart(5, "0")}`,
    firstName: isCorporate ? undefined : firstName,
    fatherName: isCorporate ? undefined : fatherName,
    grandfatherName: isCorporate ? undefined : (grandfatherName || undefined),
    familyName: isCorporate ? undefined : familyName,
    ...
    email: `${(isCorporate ? companyName! : name).toLowerCase().replace(/\s+/g, ".")}@example.com`,
    address: `Building ${int(1, 40)}, Street ${int(1, 20)}, ${branch.city}`,
    nationalId: isCorporate ? undefined : `${nationality === "Saudi" ? "1" : "2"}${int(100000000, 999999999)}`,
    nationality: isCorporate ? undefined : nationality,
    preferredLanguage,
    customerType,
    companyName,
    crNumber: isCorporate ? `CR-${int(1000000000, 9999999999)}` : undefined,
    vatNumber: isCorporate ? `3${int(100000000, 999999999)}00003` : undefined,
    contactPersonName: isCorporate ? `${firstName} ${fatherName}` : undefined,
    dateOfBirth: isCorporate ? undefined : daysAgo(int(6570, 25550)).slice(0, 10),
    gender: isCorporate ? undefined : gender,
    branchId: branch.id,
